 ...
 */
export function validateUploadFile({ filename, fileSize, mimeType, maxSizeBytes = 500 * 1024 * 1024 }) {
...
export function getMediaTypeFromMime(mimeType, filename = '') {
  if (filename && filename.toLowerCase().match(/\.cdr(\.enc)?$/i)) return 'cdr';
  if (!mimeType) return 'other';
  const mime = mimeType.toLowerCase();

  if (mime.includes('cdr') || mime.includes('coreldraw')) return 'cdr';
 ...
 */
export const SUPPORTED_PROVIDERS = ['r2', 'b2', 's3', 'minio', 'wasabi', 'custom_s3', 'local'];
