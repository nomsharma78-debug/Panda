...
let activeUserId = (function () {
  if (typeof window !== 'undefined' && window.sessionStorage) {
    try {
      return window.sessionStorage.getItem('panda_last_active_user_id') || null;
    } catch {}
  }
  return null;
})();
...
export const pandaCache = {
  ...
  setUser(userId) {
    ...
    if (nextId) {
      if (isBrowser()) {
        try {
          window.sessionStorage.setItem('panda_last_active_user_id', nextId);
        } catch {}
      }
    } else {
      if (isBrowser()) {
        try {
          window.sessionStorage.removeItem('panda_last_active_user_id');
        } catch {}
      }
    }
