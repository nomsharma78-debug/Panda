...
export class StorageManager {
  ...
  static async getMediaBinary(userId, mediaId) {
    ...
    if (media.storage_connection_id) {
      storageRecord = await getStorageConnectionInternal(media.storage_connection_id, userId);
    }
    if (!storageRecord) {
      const connections = await listUserStorageConnections(userId);
      if (connections && connections.length > 0) {
        storageRecord = await getStorageConnectionInternal(connections[0].id, userId);
      }
    }
   ...
   */
  static async syncStorageMedia(userId) {
    if (!userId) return [];
    try {
      const connections = await listUserStorageConnections(userId);
      if (!connections || connections.length === 0) return [];

      const existingMedia = await listUserMedia(userId, { limit: 1000 });
      const existingKeys = new Set(existingMedia.map((m) => m.object_key));
      const newlyDiscovered = [];

      for (const conn of connections) {
        try {
          const storageRecord = await getStorageConnectionInternal(conn.id, userId);
          ...
          if (typeof provider.listObjects === 'function') {
            ...
            for (const obj of allObjects) {
              if (!obj.key || obj.key.endsWith('/') || seenKeys.has(obj.key) || existingKeys.has(obj.key)) continue;
              seenKeys.add(obj.key);

              const cleanName = obj.key.replace(/^media\//, '') || 'Cloud File';
              const fileId = cleanName.replace(/\.enc$/, '') || generateSecureId();
