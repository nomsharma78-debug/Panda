...
export class StorageManager {
  ...
  static async uploadMedia(userId, {
    ...
    // Determine folder path prefix if uploading into a folder
    let folderPrefix = '';
    if (folderId) {
      try {
        ...
        if (rows && rows[0]?.name) {
          const safeFolderName = rows[0].name.trim().replace(/[\/\\]/g, '_');
      ...
      if (!folderPrefix) {
        try {
          const { getSupabaseMediaClient } = await import('../db/media.js');
          const sb = getSupabaseMediaClient(token);
          if (sb) {
            const { data: sbFld } = await sb.from('media_folders').select('name').eq('id', folderId).single();
            if (sbFld && sbFld.name) {
              const safeFolderName = sbFld.name.trim().replace(/[\/\\]/g, '_');
              folderPrefix = `${safeFolderName}/`;
            }
          }
        } catch {}
    ...
    // 1. Upload to Object Storage with metadata (and maintain folder marker in bucket)
    try {
      if (folderPrefix) {
        await provider.upload(`media/${folderPrefix}.keep`, Buffer.from(''), 'application/x-directory', {}).catch(() => {});
      }
  ...
  static async syncStorageMedia(userId, token = null) {
    ...
    try {
      ...
      const existingMedia = await listUserMedia(userId, { token, limit: 2000, folderId: undefined });
      const existingKeys = new Set(existingMedia.map((m) => m.object_key));
      const existingIds = new Set(existingMedia.map((m) => m.id));
      const newlyDiscovered = [];

      // Fetch user's existing folders for folder resolution and self-healing
      const { listUserFolders, createFolder } = await import('../db/folders.js');
      const userFolders = await listUserFolders(userId, { token, parentId: undefined });
      const folderMap = new Map();
      (userFolders || []).forEach((f) => {
        if (f.name) folderMap.set(f.name.toLowerCase().trim(), f);
      });

      // Self-heal existing media whose folder_id is null but objectKey has a folder prefix
      for (const m of existingMedia) {
        if (!m.folder_id && m.object_key) {
          const cleanKey = m.object_key.replace(/^media\//, '');
          const parts = cleanKey.split('/');
          if (parts.length > 1) {
            const folderName = parts[0].trim();
            let matchedFolder = folderMap.get(folderName.toLowerCase());
            if (!matchedFolder) {
              matchedFolder = await createFolder(userId, { name: folderName });
              if (matchedFolder) folderMap.set(folderName.toLowerCase(), matchedFolder);
            }
            if (matchedFolder) {
              const { moveMediaFile } = await import('../db/media.js');
              await moveMediaFile(m.id, userId, matchedFolder.id);
            }
          }
        }
      }
      ...
      for (const conn of connections) {
        try {
          ...
          if (typeof provider.listObjects === 'function') {
            ...
            for (const obj of allObjects) {
              if (!obj.key) continue;
              if (Number(obj.size) === 0 && !obj.key.endsWith('.keep') && !obj.key.endsWith('/')) {
                continue; // Skip 0-byte delete markers
              }
              totalBucketBytes += Number(obj.size) || 0;

              // Check if object key represents a folder directory marker (.keep or trailing slash)
              const cleanKey = obj.key.replace(/^media\//, '');
              const parts = cleanKey.split('/').filter(Boolean);

              if (obj.key.endsWith('/.keep') || obj.key.endsWith('.keep') || obj.key.endsWith('/')) {
                if (parts.length > 0) {
                  const folderName = parts[0].replace(/\.keep$/, '').trim();
                  if (folderName && !folderMap.has(folderName.toLowerCase())) {
                    const newFolder = await createFolder(userId, { name: folderName, storageConnectionId: conn.id });
                    if (newFolder) folderMap.set(folderName.toLowerCase(), newFolder);
                  }
                }
                continue; // Do not register .keep files as media files
              }

              // Determine if file is inside a folder in the bucket
              let resolvedFolderId = null;
              let fileBaseName = cleanKey;

              if (parts.length > 1) {
                const folderName = parts[0].trim();
                fileBaseName = parts.slice(1).join('/');
                let matchedFolder = folderMap.get(folderName.toLowerCase());
                if (!matchedFolder) {
                  matchedFolder = await createFolder(userId, { name: folderName, storageConnectionId: conn.id });
                  if (matchedFolder) folderMap.set(folderName.toLowerCase(), matchedFolder);
                }
                if (matchedFolder) {
                  resolvedFolderId = matchedFolder.id;
                }
              }
