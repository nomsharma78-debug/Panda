...
export class StorageManager {
  ...
  static async getMediaBinary(userId, mediaId, token = null) {
    ...
    return {
      ...
      filename: media?.original_filename || 'image.jpg',
        ...
        try {
          ...
          if (typeof provider.listObjects === 'function') {
            ...
            for (const obj of allObjects) {
              ...
              const uuidMatch = cleanName.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i);
              const fileId = uuidMatch ? uuidMatch[0] : (cleanName.replace(/\.enc$/, '').slice(0, 36) || generateSecureId());
              const isEnc = cleanName.endsWith('.enc');

              // Extract real original filename from cloud object key
              let realFilename = cleanName.replace(/\.enc$/, '');
              ...
              const lowerName = (realFilename || cleanName).toLowerCase();
              ...
              try {
                const record = await createMediaFile(userId, {
                  ...
                  objectKey: obj.key,
                  originalFilename: realFilename || cleanName,
                  mimeType,
                  fileSize: obj.size || 0,
                  mediaType,
                  encrypted: isEnc,
