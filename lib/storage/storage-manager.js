    ...
    if (folderId) {
      try {
        ...
        const { rows } = await dbQuery(
          `SELECT id, name FROM media_folders WHERE id = $1 AND user_id = $2`,
          [folderId, userId]
        );
        if (rows && rows[0]?.name) {
        ...
        try {
          ...
          if (typeof provider.listObjects === 'function') {
            ...
            for (const o of allObjects) {
              if (o.key && !o.key.endsWith('/.keep') && !o.key.endsWith('.keep') && !o.key.endsWith('/')) {
            ...
            for (const em of existingMedia) {
              if (em.storage_connection_id === conn.id && em.object_key && !currentBucketKeys.has(em.object_key)) {
                const stillExists = allObjects.some((o) => o.key === em.object_key || (em.id && o.key.includes(em.id)));
                if (!stillExists) {
