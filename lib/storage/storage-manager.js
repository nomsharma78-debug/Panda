...
export class StorageManager {
  ...
  static async getMediaBinary(userId, mediaId, token = null) {
    ...
    const media = await getMediaFileById(mediaId, userId, token);
    if (!media) {
      // Check if media exists for another user to report FORBIDDEN vs NOT_FOUND
      const anyMedia = await getMediaFileById(mediaId, null, token).catch(() => null);
      if (anyMedia && anyMedia.user_id !== userId) {
        console.warn('[MEDIA ACCESS] FORBIDDEN: File belongs to another user:', { mediaId, requestedBy: userId, actualOwner: anyMedia.user_id });
        throw new Error('FORBIDDEN: Media file belongs to another user');
      }
      console.warn('[MEDIA ACCESS] NOT FOUND: Media record does not exist in DB:', { mediaId, userId });
      throw new Error(`Media file not found: ${mediaId}`);
    }

    console.log('[MEDIA ACCESS] OWNERSHIP OK:', { mediaId: media.id, userId: media.user_id, filename: media.original_filename });
    ...
    console.log('[MEDIA ACCESS] STORAGE RESOLUTION START:', {
