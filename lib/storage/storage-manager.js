        ...
        connections.map(async (conn) => {
          try {
            ...
            if (!Array.isArray(allObjects) || allObjects.length === 0) {
              // If bucket is completely empty, clean up any orphaned records for this connection
              const orphans = existingMedia.filter((em) => em.storage_connection_id === conn.id);
              if (orphans.length > 0) {
                const { deleteMediaFile } = await import('../db/media.js');
                await Promise.all(orphans.map((o) => deleteMediaFile(o.id, userId).catch(() => {})));
              }
              await updateStorageUsage(userId, conn.id, { usedBytes: 0 }).catch(() => {});
              return;
            }
            ...
            // 1. Fast parallel orphan cleanup (files deleted directly from cloud bucket)
            const orphanedRecords = existingMedia.filter(
              (em) =>
                em.storage_connection_id === conn.id &&
                em.object_key &&
                !currentBucketKeys.has(em.object_key) &&
                (!em.id || !currentBucketIds.has(em.id))
            );

            if (orphanedRecords.length > 0) {
              const { deleteMediaFile } = await import('../db/media.js');
              await Promise.all(orphanedRecords.map((em) => deleteMediaFile(em.id, userId).catch(() => {})));
            }

            // 2. Discover new objects not yet in DB
            const itemsToCreate = [];
