    ...
    try {
      ...
      return {
        ...
        message: corsConfigured
          ? 'Storage connection verified successfully! Live read, write, delete, and CORS checks passed.'
          : 'Storage connection verified! (Note: CORS is not yet enabled on this bucket. Remember to set CORS in Bucket Settings for browser streaming).',
      };
    } catch (err) {
      try {
        await this.delete(testKey);
      ...
      if (name === 'InvalidAccessKeyId' || friendlyError.includes('InvalidAccessKeyId') || friendlyError.includes('key does not exist')) {
        friendlyError = 'Invalid Access Key (keyID): Your cloud provider did not recognize this Access Key ID.';
      } else if (name === 'SignatureDoesNotMatch' || friendlyError.includes('SignatureDoesNotMatch') || friendlyError.includes('secret')) {
        friendlyError = 'Invalid Secret Key (applicationKey): The secret access key is incorrect.';
      } else if (name === 'NoSuchBucket' || friendlyError.includes('NoSuchBucket') || friendlyError.includes('bucket does not exist')) {
        friendlyError = `Bucket '${this.bucket}' not found. Please verify the exact bucket name in your cloud provider console.`;
      } else if (name === 'AccessDenied' || code === 403 || friendlyError.includes('AccessDenied') || friendlyError.includes('forbidden')) {
        friendlyError = `Access Denied (403): Please ensure your Application Key has 'Read and Write' permissions for bucket '${this.bucket}'.`;
