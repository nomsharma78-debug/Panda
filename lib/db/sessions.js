  ...
  if (isSupabaseConfigured()) {
    ...
    if (supabase) {
      try {
        ...
        if (error || !userSessions || userSessions.length === 0) {
          return false; // No active sessions for this user -> Revoked!
        }

        // Match by token_hash OR matching user_agent
        const match = userSessions.find(
          (s) =>
            s.token_hash === tokenHash ||
            (s.user_agent && trimmedUa && s.user_agent.trim() === trimmedUa)
        );

        if (!match) {
          return false; // This device's session was revoked!
        }
