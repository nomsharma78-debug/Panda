...
/**
 * Record or touch a device session for a user (called on authenticated requests)
 * Ensures every connected mobile device and desktop is tracked as a distinct active session.
 * @param {string} userId
  ...
  if (isSupabaseConfigured()) {
    ...
    if (supabase) {
      try {
        ...
        // Fallback without extra columns if table structure differs
        try {
          const newId = generateSecureId();
          await supabase.from('sessions').insert([
            {
