...
export async function createMediaFile(userId, {
  ...
  if (supabase) {
    try {
      ...
      if (insErr) {
        // Try fallback without storage_connection_id or folder_id if foreign key error occurs
        const fallbackPayload = {
          ...payload,
          storage_connection_id: null,
          folder_id: null,
        };
        const { error: retryErr } = await supabase.from('media_files').insert([fallbackPayload]);
        if (retryErr) {
          console.warn('[Panda DB] Supabase media_files insert retry note:', retryErr.message);
        } else {
