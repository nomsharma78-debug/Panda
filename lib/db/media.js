...
export async function listUserMedia(userId, { token = null, mediaType = 'all', search = '', limit = 500, offset = 0 } = {}) {
  ...
  const [pgResults, sbResults] = await Promise.allSettled([fetchPg(), fetchSupabase()]);

  // 1. Process Supabase results if returned
  if (sbResults.status === 'fulfilled' && sbResults.value) {
    const { items, conns } = sbResults.value;
  ...
  // 2. Merge PostgreSQL results
  if (pgResults.status === 'fulfilled' && Array.isArray(pgResults.value)) {
    for (const pr of pgResults.value) {
      const existing = itemsMap.get(pr.id);
      if (!existing) {
        itemsMap.set(pr.id, pr);
      } else {
        if (!existing.storage_connection_id && pr.storage_connection_id) {
          existing.storage_connection_id = pr.storage_connection_id;
        }
        if (pr.storage_name && !existing.storage_name) {
          existing.storage_name = pr.storage_name;
          existing.storage_provider = pr.storage_provider;
        }
      }
    }
  }
  ...
  allItems.sort((a, b) => new Date(b.uploaded_at || b.created_at) - new Date(a.uploaded_at || a.created_at));

  return allItems.map((item) => formatMediaItem(item, storageMap));
}
