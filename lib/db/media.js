  ...
  if (folderId) {
    ...
    try {
      const { rows: folderRows } = await query(
        `SELECT id, user_id, storage_connection_id, name, parent_id, color, created_at, updated_at
         FROM media_folders WHERE id = $1 AND user_id = $2`,
        [folderId, userId]
      );
    ...
    if (sbClient) {
      try {
        await sbClient.from('media_folders').upsert(
          {
            id: folderId,
            user_id: userId,
            storage_connection_id: folderStorageConnId || storageConnectionId || null,
            name: folderName,
        ...
        // Fallback without storage_connection_id if constraint fails
        try {
          await sbClient.from('media_folders').upsert(
            {
              id: folderId,
    ...
    try {
      ...
      if (insErr) {
        ...
        if (!retry1.error) {
          ...
          // Under NO CIRCUMSTANCE strip folder_id. PostgreSQL will preserve the folder_id cleanly!
        }
      } else {
        supabaseInserted = true;
      }
