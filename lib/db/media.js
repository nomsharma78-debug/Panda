...
export async function listUserMedia(userId, { token = null, mediaType = 'all', search = '', limit = 500, offset = 0 } = {}) {
    ...
    try {
      ...
      if (mediaType && mediaType !== 'all') {
        if (mediaType === 'photo' || mediaType === 'image') {
          ...
          pgSql += ` AND (mf.media_type = 'cdr' OR mf.original_filename ILIKE '%.cdr')`;
  ...
  const pgResults = await fetchPg();
  if (pgResults && pgResults.length > 0) {
    return pgResults.map((item) => formatMediaItem(item, {}));
  }

  const sbResults = await fetchSupabase();
  const { items, conns } = sbResults || { items: [], conns: [] };
  for (const c of conns || []) {
    storageMap[c.id] = c;
  }
  const formatted = (items || []).map((item) => formatMediaItem(item, storageMap));
...
export async function getMediaStats(userId, token = null) {
  ...
  try {
    const { rows: sqlRows } = await query(
      `SELECT id, media_type, file_size FROM media_files WHERE user_id = $1`,
      [userId]
    );
    ...
    if (sqlRows && sqlRows.length > 0) {
      sqlRows.forEach((r) => {
        ...
        if (type === 'photo' || type === 'image') stats.images++;
    ...
    try {
      ...
      if (!error && sbData && sbData.length > 0) {
        sbData.forEach((item) => {
          ...
          stats.total++;
          stats.totalBytes += size;
          if (mime.startsWith('image/') || name.match(/\.(jpg|jpeg|png|webp|gif|svg|bmp|heic|avif)$/i)) {
            stats.images++;
          } else if (mime.startsWith('video/') || name.match(/\.(mp4|webm|mov|mkv|avi|m4v|3gp|flv|wmv)$/i)) {
            stats.videos++;
          } else if (mime.startsWith('audio/') || name.match(/\.(mp3|wav|ogg|m4a|aac|flac)$/i)) {
            stats.audio++;
          } else {
            stats.documents++;
          }
