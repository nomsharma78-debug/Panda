...
export async function createMediaFile(userId, {
  ...
  try {
    ...
    if (!supabaseInserted) {
      console.error('[Panda DB] Both Supabase and PostgreSQL insert failed:', pgErr.message);
    }
  }
 ...
 */
export async function listUserMedia(userId, { token = null, mediaType = 'all', folderId = undefined, search = '', limit = 200, offset = 0 } = {}) {
  if (!userId) return [];
