...
export async function createMediaFile(userId, {
  ...
  if (supabase) {
    try {
      ...
      if (insErr) {
        // Step 1: Retry with storage_connection_id stripped, preserving folder_id!
        ...
        if (!retry1.error) {
          ...
          // Step 2: Retry with both stripped if folder_id also doesn't exist
          const retry2 = await supabase.from('media_files').insert([{
            ...payload,
            storage_connection_id: null,
            folder_id: null,
          }]);
          if (!retry2.error) {
            ...
            console.warn('[Panda DB] Supabase media_files insert retry note:', retry2.error.message);
          }
        }
      } else {
        supabaseInserted = true;
