...
export async function createMediaFile(userId, {
  ...
  const isUuid = customId && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(customId);
  const id = isUuid ? customId : generateSecureId();
  ...
  if (supabase) {
    try {
      ...
      if (encryptionMetadata?.iv) sbPayload.encryption_iv = encryptionMetadata.iv;
      if (encryptionMetadata?.authTag) sbPayload.encryption_auth_tag = encryptionMetadata.authTag;

      let { error: insErr } = await supabase.from('media_files').insert([sbPayload]);
      if (insErr) {
        // Retry with minimal columns if foreign keys / optional columns errored
        ...
        const retry1 = await supabase.from('media_files').insert([minimalPayload]);
        if (!retry1.error) {
...
function formatMediaItem(item, storageMap = {}) {
  ...
  if (!mediaType) {
    if (mime.startsWith('image/') || lowerName.match(/\.(jpg|jpeg|png|webp|gif|svg|bmp|heic|avif)$/i)) {
      mediaType = 'photo';
    } else if (mime.startsWith('video/') || lowerName.match(/\.(mp4|webm|mov|mkv|avi|m4v|3gp|flv|wmv)$/i)) {
      mediaType = 'video';
    } else if (mime.startsWith('audio/') || lowerName.match(/\.(mp3|wav|ogg|m4a|aac|flac)$/i)) {
      mediaType = 'audio';
    } else if (lowerName.endsWith('.pdf') || mime.includes('pdf')) {
      mediaType = 'pdf';
    } else if (lowerName.match(/\.(zip|rar|7z|tar|gz)$/i)) {
      mediaType = 'archive';
    } else {
      mediaType = 'document';
    }
 ...
 */
export async function listUserMedia(userId, { token = null, mediaType = 'all', search = '', limit = 500, offset = 0 } = {}) {
  if (!userId) return [];

  let storageMap = {};

  // Execute PostgreSQL and Supabase in parallel for maximum speed
  const fetchPg = async () => {
    try {
      let pgSql = `
      ...
      if (mediaType && mediaType !== 'all') {
        if (mediaType === 'photo' || mediaType === 'image') {
          pgSql += ` AND (mf.media_type = 'photo' OR mf.media_type = 'image' OR mf.mime_type LIKE 'image/%')`;
        } else if (mediaType === 'video') {
          pgSql += ` AND (mf.media_type = 'video' OR mf.mime_type LIKE 'video/%')`;
        } else if (mediaType === 'pdf') {
          pgSql += ` AND (mf.media_type = 'pdf' OR mf.original_filename ILIKE '%.pdf' OR mf.mime_type LIKE '%pdf%')`;
        } else if (mediaType === 'archive') {
          pgSql += ` AND (mf.media_type = 'archive' OR mf.original_filename ILIKE '%.zip' OR mf.original_filename ILIKE '%.rar' OR mf.original_filename ILIKE '%.7z' OR mf.original_filename ILIKE '%.tar%' OR mf.original_filename ILIKE '%.gz')`;
        } else {
      ...
      const { rows } = await query(pgSql, pgParams);
      return rows || [];
    } catch {
      return [];
    }
  ...
  const fetchSupabase = async () => {
    const supabase = getSupabaseMediaClient(token);
    if (!supabase) return { items: [], conns: [] };
    try {
      let queryBuilder = supabase
        .from('media_files')
        .select(`
          id, user_id, storage_connection_id, folder_id, object_key, original_filename,
          mime_type, file_size, uploaded_at, storage_provider, status
        `)
        .eq('user_id', userId);
      ...
      const [mRes, cRes] = await Promise.allSettled([
        queryBuilder,
        supabase.from('storage_connections').select('id, name, provider').eq('user_id', userId),
      ]);

      const items = (mRes.status === 'fulfilled' && !mRes.value.error && mRes.value.data) || [];
      const conns = (cRes.status === 'fulfilled' && !cRes.value.error && cRes.value.data) || [];
      return { items, conns };
    } catch {
      return { items: [], conns: [] };
    }
  ...
  const [pgResults, sbResults] = await Promise.all([fetchPg(), fetchSupabase()]);

  const itemsMap = new Map();
  const { items: sbItems, conns: sbConns } = sbResults || { items: [], conns: [] };

  for (const c of sbConns || []) {
    storageMap[c.id] = c;
  }

  // 1. Add PostgreSQL items
  for (const item of (pgResults || [])) {
    if (item.storage_connection_id && !storageMap[item.storage_connection_id] && item.storage_name) {
      storageMap[item.storage_connection_id] = {
        id: item.storage_connection_id,
        name: item.storage_name,
        provider: item.storage_provider,
      };
    }
    itemsMap.set(item.id, formatMediaItem(item, storageMap));
  }

  // 2. Add Supabase items (merge by ID so existing items are never dropped)
  ...
  let formatted = Array.from(itemsMap.values());

  if (mediaType && mediaType !== 'all') {
    formatted = formatted.filter((item) => {
      const type = (item.media_type || '').toLowerCase();
      if (mediaType === 'photo' || mediaType === 'image') return type === 'photo' || type === 'image';
...
export async function getMediaStats(userId, token = null) {
  ...
  const countItem = (id, type, size, mime, name) => {
    ...
    if (t === 'photo' || t === 'image' || m.startsWith('image/') || n.match(/\.(jpg|jpeg|png|webp|gif|svg|bmp|heic|avif)$/i)) {
      stats.images++;
    } else if (t === 'video' || m.startsWith('video/') || n.match(/\.(mp4|webm|mov|mkv|avi|m4v|3gp|flv|wmv)$/i)) {
      stats.videos++;
    } else if (t === 'audio' || m.startsWith('audio/') || n.match(/\.(mp3|wav|ogg|m4a|aac|flac)$/i)) {
      stats.audio++;
    } else {
      stats.documents++;
    }
  ...
  try {
    const { rows: sqlRows } = await query(
      `SELECT id, media_type, file_size, mime_type, original_filename FROM media_files WHERE user_id = $1`,
      [userId]
    );

    if (sqlRows && sqlRows.length > 0) {
      sqlRows.forEach((r) => {
        countItem(r.id, r.media_type, r.file_size, r.mime_type, r.original_filename);
      });
    }
  } catch {}
  ...
  if (supabase) {
    try {
      const { data: sbData, error } = await supabase
        .from('media_files')
        .select('id, media_type, mime_type, file_size, original_filename')
        .eq('user_id', userId);
      if (!error && sbData && sbData.length > 0) {
        sbData.forEach((item) => {
          countItem(item.id, item.media_type, item.file_size, item.mime_type, item.original_filename);
        });
      }
    } catch {}
  }
 ...
 */
export async function getRecentMedia(userId, limit = 6, token = null) {
  return listUserMedia(userId, { token, limit });
}
