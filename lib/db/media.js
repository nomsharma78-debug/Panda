...
export async function listUserMedia(userId, { token = null, mediaType = 'all', folderId = undefined, search = '', limit = 500, offset = 0 } = {}) {
  ...
  if (folderId && folderId !== 'all' && folderId !== 'root') {
    try {
      const { rows: fRows } = await query(`SELECT name FROM media_folders WHERE id = $1`, [folderId]);
    ...
    if (!folderNameForLookup) {
      try {
        ...
        if (sb) {
          ...
          if (sbFld && sbFld.name) {
            folderNameForLookup = sbFld.name.toLowerCase().trim();
          }
        }
      } catch {}
    }
  ...
  if (supabase) {
    try {
      ...
      console.warn('[Panda DB] Supabase listUserMedia notice:', sbErr.message);
    }
  }

  // 2. Fetch from PostgreSQL to merge and ensure dual-DB consistency
  try {
    let pgSql = `
    ...
    if (pgRows && pgRows.length > 0) {
      for (const pr of pgRows) {
        ...
        if (!existing) {
          ...
          // Merge authoritative fields from PostgreSQL if Supabase had null
          if (!existing.folder_id && pr.folder_id) {
            existing.folder_id = pr.folder_id;
            if (supabase) {
              supabase.from('media_files').update({ folder_id: pr.folder_id }).eq('id', pr.id).then(() => {}).catch(() => {});
            }
          }
  ...
  const isolatedItems = applyFolderIsolation(allItems);
  return isolatedItems.map((item) => formatMediaItem(item, storageMap));
}
