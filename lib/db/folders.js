  ...
  if (supabase) {
    try {
      ...
      if (data) {
        ...
        try {
          const { rows: pgCounts } = await query(
          ...
          if (pgCounts && pgCounts.length > 0) {
            for (const r of pgCounts) {
              const pgCnt = parseInt(r.cnt || '0', 10);
              const pgSz = parseInt(r.sz || '0', 10);
              if (!fileCounts[r.folder_id] || fileCounts[r.folder_id].count < pgCnt) {
                fileCounts[r.folder_id] = { count: pgCnt, size: pgSz };
              }
            }
          }
        } catch {}
