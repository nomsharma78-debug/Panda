...
async function getVaultPostgresPool() {
  ...
  const connString = (process.env.VAULT_DATABASE_URL || process.env.DATABASE_URL || '').trim();
...
async function getAuthPostgresPool() {
  ...
  const connString = (process.env.AUTH_DATABASE_URL || process.env.DATABASE_URL || '').trim();
  if (!connString) return null;
...
export async function queryVault(sql, params = []) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL) {
      throw new Error(`PostgreSQL Vault connection unavailable (${lastVaultError || 'DATABASE_URL not configured or connection failed'}).`);
    }
...
export async function queryAuth(sql, params = []) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL) {
      throw new Error(`PostgreSQL Auth connection unavailable (${lastAuthError || 'DATABASE_URL or AUTH_DATABASE_URL not configured or connection failed'}).`);
    }
