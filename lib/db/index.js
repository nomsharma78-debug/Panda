...
export async function queryVault(sql, params = []) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL) {
      throw new Error('PostgreSQL connection pool is unavailable. Please check your DATABASE_URL or SUPABASE_SERVICE_ROLE_KEY.');
    }
...
export async function queryAuth(sql, params = []) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL) {
      throw new Error('PostgreSQL Auth pool is unavailable. Please check your DATABASE_URL or SUPABASE_SERVICE_ROLE_KEY.');
    }
