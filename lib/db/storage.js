...
export async function createStorageConnection(userId, { token = null, provider, name, bucket = 'default', region = null, endpoint = null, encryptedConfig, isDefault = false }) {
  ...
  // 1. Try Supabase Client (authenticated with user token or admin)
  const supabase = getSupabaseStorageClient(token);
  if (supabase) {
    try {
      if (isDefault) {
 ...
 */
export async function getSafeStorageConnection(id, userId, token = null) {
  if (!id || !userId) return null;

  const item = await getStorageConnectionInternal(id, userId, token);
  if (!item) return null;
 ...
 */
export async function deleteStorageConnection(id, userId, token = null) {
