...
export async function validateStorageEndpoint(endpointUrl, allowHttpInDev = true) {
  ...
  try {
    ...
    if (!addresses || addresses.length === 0) {
      return { valid: false, error: `The hostname '${hostname}' could not be found. Please check your endpoint URL.` };
    }

    for (const record of addresses) {
      if (record.family === 4 && isPrivateIPv4(record.address)) {
        return { valid: false, error: `Endpoint resolves to private network address (${record.address}). Private networks are not supported.` };
      }
      if (record.family === 6 && isPrivateIPv6(record.address)) {
        return { valid: false, error: `Endpoint resolves to private IPv6 address (${record.address}).` };
      }
    }
  } catch {
    return { valid: false, error: `The endpoint hostname '${hostname}' could not be found. Please verify your S3 Endpoint URL (e.g. s3.us-west-004.backblazeb2.com).` };
  }
